(() => {
  ...
  const key = 'sibt.options.paper.v2';
  const legacyKey = 'sibt.mstr.paper.v1';
  const positionsKey = 'sibt.options.positions.v2';
  const legacyPositionKey = 'sibt.mstr.position.v1';
  ...
  let journal = [], positions = {}, storageError = '';
  try {
    const saved = JSON.parse(localStorage.getItem(key) || 'null');
    const legacy = JSON.parse(localStorage.getItem(legacyKey) || '[]');
    journal = Array.isArray(saved) ? saved : Array.isArray(legacy) ? legacy.map(entry => ({...entry,symbol:entry.symbol || 'MSTR'})) : [];
    positions = JSON.parse(localStorage.getItem(positionsKey) || '{}') || {};
    const legacyPosition = JSON.parse(localStorage.getItem(legacyPositionKey) || 'null');
  ...
  const fields = [
    ...
    ['spot','Stock quote price ($)','number','','0.01','0.01'],
  ...
  function restorePosition(symbol = activeSymbol) {
    try {
      form.elements.spot.value = '';
      form.elements.asOf.value = '';
      form.elements.standard.checked = false;
  ...
  function addRow(values = {}) {
    ...
    tr.dataset.delta = values.delta ?? '';
    tr.dataset.iv = values.iv ?? '';
    tr.dataset.volume = values.volume ?? '';
  ...
  async function loadChain({probe = false} = {}) {
    ...
    const symbol = ticker(form.elements.symbol.value);
    if (!probe && !validTicker(symbol)) { status.textContent = 'Enter a valid stock or ETF ticker.'; form.elements.symbol.focus(); return; }
    if (!probe && symbol !== activeSymbol) { activeSymbol = symbol; localStorage.setItem(lastSymbolKey, symbol); updateSymbolLabels(); }
    ...
    status.textContent = probe ? 'Checking automatic feed…' : `Loading current ${symbol} calls…`;
    try {
      const query = probe ? new URLSearchParams({probe:'1',symbol:activeSymbol}) : new URLSearchParams({ symbol, minDte: form.elements.minDte.value || '7', maxDte: form.elements.maxDte.value || '45', minStrike: String(minStrike || 0) });
  ...
  function analyze() {
    ...
    const ranking = CoveredCall.rankCandidates(result, position);
  ...
  function renderJournal() {
    ...
    panel.innerHTML = `<p class="desk-note">${open.length} open · ${closed.length} closed · Closed-trade P&amp;L from entry: ${money(closed.reduce((sum,j) => sum + j.closed.result.totalPnl, 0))}. Open positions are unmarked and excluded; this is not total portfolio performance.</p>` + symbolJournal.slice().reverse().map(j => `<article class="subcard desk-candidate"><h4>${esc(j.symbol)} · ${esc(j.call.expiration)} · $${esc(j.call.strike)} call · ${esc(j.position.contracts)} contract(s) · ${j.closed ? 'CLOSED' : 'OPEN (not marked)'}</h4><p class="desk-note">Paper entry ${esc(new Date(j.openedAt).toLocaleString())} · Source ${esc(j.position.source)} · Net entry premium ${money(j.metrics.netPremium)}</p>${j.closed ? `<div class="kv"><span>Stock P&amp;L from entry</span><span>${money(j.closed.result.stockPnl)}</span></div><div class="kv"><span>Option P&amp;L after fees</span><span>${money(j.closed.result.optionPnl)}</span></div><div class="kv"><strong>Combined P&amp;L from entry</strong><strong>${money(j.closed.result.totalPnl)}</strong></div><div class="kv"><span>Hold-only benchmark</span><span>${money(j.closed.result.holdPnl)}</span></div><div class="kv"><span>Combined P&amp;L since cost basis</span><span>${money(j.closed.result.totalPnlCost)}</span></div>` : `<form data-close="${esc(j.id)}" class="desk-close"><p class="desk-note">Simulate closing both legs at the same time. Enter the stock sale price and call buyback ask, even if the call is in the money. This does not model assignment or automatic expiry.</p><div class="desk-fields"><label>${esc(j.symbol)} exit price ($)<input name="stock" type="number" min="0" step="0.01" required></label><label>Call buyback / share ($)<input name="buyback" type="number" min="0" step="0.01" required></label><label>Closing fee / contract ($)<input name="fee" type="number" min="0" step="0.01" value="0.65" required></label></div><button type="submit">Record paper close</button></form>`}</article>`).join('');
