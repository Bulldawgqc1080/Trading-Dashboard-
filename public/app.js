...
function renderJournal(data) {
  ...
  status.textContent = `${data.count} independent current-model trading day${data.count === 1 ? '' : 's'}${Number.isFinite(data.rawHistoricalCount) ? ` · ${data.rawHistoricalCount} raw historical rows retained` : ''}`;
  panel.innerHTML = `<div class="journal-list">${data.journal.slice(-6).reverse().map(j => { const col = j.decision === 'YES' ? 'var(--green)' : j.decision === 'CAUTION' ? 'var(--amber)' : 'var(--red)'; return `<div class="journal-item"><div class="kv"><span style="color:${col};font-weight:700">${j.date} · ${j.decision || '—'}</span><span>score ${j.score ?? '—'}</span></div><div class="kv"><span>Input confidence</span><span>${j.confidenceScore ?? '—'}</span></div><div class="kv"><span>SPY</span><span>${j.spyEntry ?? '—'}</span></div><div class="kv"><span>1D / 5D / 10D</span><span>${j.outcome1d ?? '—'} / ${j.outcome5d ?? '—'} / ${j.outcome10d ?? '—'}</span></div><div>${(j.topReasons || []).map(r => pill(r, 'good')).join('')}</div></div>`; }).join('')}</div><div style="margin-top:8px;font-size:10px;color:var(--text3);">One independent market-open observation per current-model trading day. Raw legacy rows remain stored for audit but are not shown or counted here.</div>`;
}
