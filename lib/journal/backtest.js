...
function independentDailyEntries(entries) {
  const byDay = new Map();
  (entries || []).forEach((entry, index) => {
    const key = entry.date ? `${entry.modelVersion || 'unknown'}:${entry.date}` : `undated:${index}`;
    const existing = byDay.get(key);
    if (!existing || Number(entry.ts || 0) >= Number(existing.ts || 0)) byDay.set(key, entry);
  });
  return [...byDay.values()];
}
...
function buildValidationReport(entries) {
  const verified = independentDailyEntries(entries.filter(j => j.modelVersion === MODEL_VERSION && j.outcomeMethod === 'trading-date-v1' && j.validationEligible === true));
...
function buildBacktestSummary() {
  ...
  const verified = independentDailyEntries(normalized.filter(j => j.modelVersion === MODEL_VERSION && j.outcomeMethod === 'trading-date-v1' && j.validationEligible === true));
...
module.exports = { backfillJournalOutcomes, buildBacktestSummary, summarizeDecisionBucket, summarizeConfidenceBuckets, buildValidationReport, independentDailyEntries };
