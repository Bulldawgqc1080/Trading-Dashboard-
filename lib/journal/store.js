...
function normalizeJournalEntry(j) {
  return {
    ...
    outcomeMethod: j.outcomeMethod ?? null,
    validationEligible: j.validationEligible ?? null
  };
}
...
async function logJournalEntry(entry) {
  const now = Date.now();
  const today = entry.date || new Intl.DateTimeFormat('en-CA', { timeZone:'America/New_York', year:'numeric', month:'2-digit', day:'2-digit' }).format(new Date(now));
  await loadJournal();
  ...
  const recent = [...journal].reverse().find(j => {
    ...
    return sameKey || (closeInTime && sameShape);
  });
...
function replaceJournal(entries) {
  journal = Array.isArray(entries) ? entries.map(normalizeJournalEntry) : [];
  return getJournal();
}

module.exports = { loadJournal, saveJournal, logJournalEntry, getJournal, replaceJournal, normalizeJournalEntry };
