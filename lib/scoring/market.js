...
function scoreMacro(d) {
  ...
  else if (d.scheduledEvents?.highImpact24hr) macro -= 12;
...
function buildReasonsAndBlockers(d, permission, spread) {
  ...
  else if (d.scheduledEvents?.highImpact24hr && d.scheduledEvents.nearest) blockers.push(`${d.scheduledEvents.nearest.label} risk close`);
...
function buildMarketScore(d) {
  const volatility = scoreVolatility(d), trend = scoreTrend(d), momentum = scoreMomentum(d), macro = scoreMacro(d);
  let weightedScore = Math.round(volatility * 0.25 + momentum * 0.30 + trend * 0.35 + macro * 0.10);
  const vetoFlags = [];
  ...
  validationWarnings.push('breadth is excluded until a verified breadth feed is available');
  validationWarnings.push(`scheduled-event calendar verified through ${d.scheduledEvents?.coverageThrough || 'unknown'}`);
  ...
  return {
    ...
    categoryScores: { volatility, trend, momentum, macro },
    weightedScore,
    decision,
    permissionLabel,
    vetoFlags,
