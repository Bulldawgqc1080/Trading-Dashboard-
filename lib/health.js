...
function getFeedQuality(feedHealth = {}) {
  ...
  if (errors.length > 0 || stale.length > 3) {
  ...
  return { quality: 'good', label: 'FEEDS RESPONDING', errors: [], stale: [], criticalDown: [] };
}
...
function buildSystemStatus({ marketData, feedQuality }) {
  ...
  if (marketData?.indicatorHistoryComplete === false) missingCritical.push('required moving-average history');
