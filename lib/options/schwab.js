...
async function buildSchwabChain({ request = schwabGet, accessToken, symbol = 'MSTR', minDte = 7, maxDte = 45, minStrike = 0, now = Date.now() }) {
  ...
  const [data, quoteData] = await Promise.all([
    request('/marketdata/v1/chains', {
      symbol, contractType: 'CALL', strategy: 'SINGLE', includeUnderlyingQuote: 'true',
...
module.exports = { schwabGet, normalizeSchwabOption, standardContract, flattenCalls, buildSchwabChain, buildSchwabMstrChain: buildSchwabChain };
